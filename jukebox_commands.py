    ...
    async def fw(self, ctx, userid):
        ...
        try:
            ...
            embed = discord.Embed(title=f"{userPing} ({userNick})",
                                  description=f"User not in Voice Chat\n\nRoles:\n{formatted_roles}", colour=0x00b0f4,
        ...
        # victim in vc data
        if not member.voice.mute:
            view_items['serverMute'] = Button(label="Server Mute", style=discord.ButtonStyle.red,
                                              custom_id="servermute")
        if member.voice.mute:
            userMuted = "Server Muted"
            view_items['serverMute'] = Button(label="Server Unmute", style=discord.ButtonStyle.green,
                                              custom_id="serverunmute")
            ...
            userMuted = "Error"
        if not member.voice.deaf:
            view_items['serverDeafen'] = Button(label="Server Deafen", style=discord.ButtonStyle.red,
                                                custom_id="serverdeafen")
        if member.voice.deaf:
            userDeafened = "Server Deafened"
            view_items['serverDeafen'] = Button(label="Server Undeafen", style=discord.ButtonStyle.green,
                                                custom_id="serverundeafen")
...
def get_current_track_embed(guild: discord.Guild, show_tracking: bool, description: Optional[str] = None, previous_track: JukeboxItem = None) -> discord.Embed:
    ...
    if not current:
        ...
        # embed icon
        thumbnail_url = emoji.url if emoji and show_tracking else current.thumbnail
